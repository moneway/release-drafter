...
const getCategoriesConfig = ({ config }) => {
  ...
  const orderedCategories = [
    UNCATEGORIZED,
    ..._.flatMap(config.categories, category => category.title)
  ]
  ...
  return pullRequests.reduce(
    (acc, pullRequest) => {
      ...
      if (labels.some(label => excludeLabels.includes(label.name))) {
        ...
        let matchedCategories = orderedCategories.filter(category =>
          labels.some(label => {
            if (categoriesConfig[category]) {
              return categoriesConfig[category].labels.includes(label.name)
            }
            return false
          })
        )
